...
struct IoctlPeer {
    IoctlPeerFlags      Flags;              // 4 bytes
    uint32_t            Reserved;           // 4 bytes
    uint8_t             PublicKey[32];      // 32 bytes
    uint8_t             PresharedKey[32];   // 32 bytes
    uint16_t            PersistentKeepalive;// 2 bytes
    // No explicit padding needed here - Pack=8 will align naturally
    SOCKADDR_INET       Endpoint;           // Typically 28 bytes on Windows
    uint64_t            TxBytes;            // 8 bytes
    uint64_t            RxBytes;            // 8 bytes
    uint64_t            LastHandshake;      // 8 bytes
    uint32_t            AllowedIPsCount;    // 4 bytes
    // Padding to reach exactly 136 bytes
    uint8_t             _pad[4];            // 4 bytes (makes total 136)
};
// Verify size at compile time
static_assert(sizeof(IoctlPeer) == 136, "IoctlPeer size mismatch");
...
struct IoctlAllowedIP {
    union {
        IN_ADDR  V4;                        // offset 0
        IN6_ADDR V6;                        // offset 0
    };                                      // union size = 16 bytes

    // No padding at offset 16
    ADDRESS_FAMILY      AddressFamily;      // offset 16 (2 bytes)

    // 2 bytes of implicit padding here (offsets 18-19)

    uint8_t             Cidr;               // offset 20 (1 byte)

    // 3 bytes of padding to reach size 24 (offsets 21-23)
    uint8_t             _pad[3];
};

static_assert(sizeof(IoctlAllowedIP) == 24, "Size mismatch");
